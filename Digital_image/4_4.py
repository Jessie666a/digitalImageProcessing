...
def myDFTandGLPF(myImg):
    ...
    for u in range(width):
        for v in range(height):
            H[u, v] = np.exp(-((u - height // 2) ** 2 + (v - width // 2) ** 2) / (2 * D0 ** 2))

    # 高斯低通滤波
    GLPF = H * dftShift
    # 5. 去中心化
    iShift = np.fft.ifftshift(GLPF)
    # 6. 傅里叶逆变换
    idft = cv.idft(iShift)
    # 7. 重建图像
...
GLPFimg = myDFTandGLPF(grayImg)

# 循环显示
imgList = [grayImg, GLPFimg[0], GLPFimg[4][:, :, 0]]
...
ax.plot_surface(u, v, GLPFimg[4][:, :, 0], cmap='viridis')
plt.axis('off')
